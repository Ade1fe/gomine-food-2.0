...
import 'package:path_provider/path_provider.dart';
import 'app_scaffold.dart';
import 'recipe_file_handler.dart';
...
class _RecipeLibraryScreenState extends State<RecipeLibraryScreen> {
  ...
  Future<void> _openRecipe(String fileName) async {
    final success = await RecipeFileHandler.openRecipeFile(context, fileName);

    if (!success) {
      // If file wasn't found or couldn't be opened, show a more helpful message
      // ignore: use_build_context_synchronously
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: const Text(
            'Recipe not found. It may have been moved or deleted. Try downloading it again.',
          ),
          backgroundColor: Colors.red,
