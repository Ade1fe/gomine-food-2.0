...
//         debugPrint('Loaded ${favoriteRecipes.length} favorite recipes: $favoriteRecipes');
//       } else {
//         debugPrint('Favorites file does not exist');
//       }
...
//     debugPrint('Building favorite card for ${recipe['name']}, currentMeal: $currentMeal');
...
//         borderRadius: BorderRadius.circular(16),
//         onTap: () {
//           debugPrint('Tapped on favorite recipe: ${recipe['name']}');
//           Navigator.of(context).pop();
//           if (!currentMeal) {
//             Navigator.push(
//               context,
...
//       debugPrint('Adding to favorites: $recipeMetadata');
...
//         debugPrint('Existing favorites: $existingFavorites');
//       }
...
//       if (recipeExists) {
//         debugPrint('Recipe already exists in favorites');
...
//       debugPrint('Favorites saved successfully: ${existingFavorites.length} recipes');
//     } catch (e) {
...
//         ScaffoldMessenger.of(context).showSnackBar(
//           SnackBar(
//             content: const Text('Preparing recipe to share...'),
...
//       // Option 1: Share recipe link if available
//       if (recipeDetails['strSource'] != null &&
//           recipeDetails['strSource'].toString().isNotEmpty) {
//         await Share.share(
//           'Check out this amazing recipe for ${widget.mealName}!\n\n${recipeDetails['strSource']}',
//           subject: 'Recipe: ${widget.mealName}',
//         );
//         return;
//       }

//       // Option 2: Share recipe details as text
...
//           'Category: ${recipeDetails['strCategory'] ?? 'Main Course'}');
//       recipeText
//           .writeln('Cuisine: ${recipeDetails['strArea'] ?? 'International'}');
//       recipeText.writeln('');

//       // Add ingredients
//       recipeText.writeln('INGREDIENTS:');
//       for (int i = 0; i < ingredients.length; i++) {
//         recipeText.writeln('• ${ingredients[i]} - ${measures[i]}');
//       }
//       recipeText.writeln('');

//       // Add instructions
//       recipeText.writeln('INSTRUCTIONS:');
//       recipeText.writeln(recipeDetails['strInstructions']);

//       // Add source if available
//       if (recipeDetails['strYoutube'] != null &&
//           recipeDetails['strYoutube'].toString().isNotEmpty) {
//         recipeText.writeln('');
//         recipeText.writeln('Video Tutorial: ${recipeDetails['strYoutube']}');
//       }
...
//               onPressed: () async {
//                 try {
//                   // For Android, use a more reliable method to open PDF
//                   if (Platform.isAndroid) {
//                     final result = await OpenFile.open(file.path);
//                     if (result.type != ResultType.done) {
//                       // If OpenFile fails, try to use Intent to open PDF
...
//                     }
//                   } else {
//                     // For iOS, use the regular OpenFile
...
//               if (await favoritesFile.exists()) {
//                 final content = await favoritesFile.readAsString();
//                 debugPrint('Favorites file content: $content');
                
//                 // ignore: use_build_context_synchronously
//                 ScaffoldMessenger.of(context).showSnackBar(
//                   SnackBar(
//                     content: Text('Favorites file exists with ${json.decode(content).length} recipes'),
//                     backgroundColor: Colors.green,
//                   ),
//                 );
//               } else {
...
//                 ScaffoldMessenger.of(context).showSnackBar(
//                   const SnackBar(
//                     content: Text('Favorites file does not exist'),
//                     backgroundColor: Colors.red,
//                   ),
//                 );
//               }
...
//         ScaffoldMessenger.of(context).showSnackBar(
//           const SnackBar(
//             content: Text('Preparing recipe to share...'),
//             duration: Duration(seconds: 1),
//             backgroundColor: Colors.deepOrange,
//           ),
//         );
...
//       // First check if the recipe file exists
//       Directory? directory;
//       if (Platform.isAndroid) {
//         directory = Directory('/storage/emulated/0/Download');
//         if (!await directory.exists()) {
...
//       // Create a sanitized filename (same as in _downloadRecipe)
//       final fileName = '${widget.mealName.replaceAll(RegExp(r'[^\w\s]+'), '').replaceAll(' ', '_')}_recipe.pdf';
//       final file = File('${directory!.path}/$fileName');

//       // If file doesn't exist, create it first
//       if (!await file.exists()) {
//         // Show message that we need to create the file first
//         if (mounted) {
//           ScaffoldMessenger.of(context).showSnackBar(
//             const SnackBar(
//               content: Text('Creating recipe PDF for sharing...'),
...
//         // Call download recipe to create the file
//         await _downloadRecipe();
//       }
...
//           [XFile(file.path)],
//           text: 'Check out this amazing recipe for ${widget.mealName}!',
//           subject: 'Recipe: ${widget.mealName}',
//         );
...
//         }
//       } else {
//         // Fallback to sharing text if file creation failed
//         if (mounted) {
//           ScaffoldMessenger.of(context).showSnackBar(
//             const SnackBar(
//               content: Text('Could not create PDF file to share. Sharing recipe details instead.'),
...
//         // Add category and cuisine
//         recipeText.writeln('Category: ${recipeDetails['strCategory'] ?? 'Main Course'}');
//         recipeText.writeln('Cuisine: ${recipeDetails['strArea'] ?? 'International'}');
//         recipeText.writeln('');
...
//       // Define theme colors
//       final PdfColor primaryColor = PdfColor.fromHex('#FF5722'); // Deep Orange
//       final PdfColor accentColor = PdfColor.fromHex('#FBE9E7'); // Light Orange
//       final PdfColor textColor = PdfColor.fromHex('#424242'); // Dark Grey
//       final PdfColor lightTextColor = PdfColor.fromHex('#757575'); // Medium Grey
...
//                 // Main content
//                 pw.Container(
//                   padding: const pw.EdgeInsets.only(top: 100, left: 40, right: 40),
//                   child: pw.Column(
//                     crossAxisAlignment: pw.CrossAxisAlignment.center,
//                     children: [
//                       // Recipe card
...
//                 // Main content
//                 pw.Container(
//                   padding: const pw.EdgeInsets.fromLTRB(80, 40, 40, 40),
//                   child: pw.Column(
//                     crossAxisAlignment: pw.CrossAxisAlignment.start,
//                     children: [
//                       // Page title
...
//                             decoration: pw.BoxDecoration(
//                               color: index % 2 == 0 ? accentColor : PdfColors.white,
...
//         final currentPageSteps = instructionSteps.sublist(i, endIdx);
//         final pageNumber = 3 + (i ~/ stepsPerPage);
...
//                   // Main content
//                   pw.Container(
//                     padding: const pw.EdgeInsets.fromLTRB(40, 40, 80, 40),
//                     child: pw.Column(
//                       crossAxisAlignment: pw.CrossAxisAlignment.start,
//                       children: [
//                         // Page title
...
//       // Add a final page with additional information if available
//       if (recipeDetails['strYoutube'] != null || recipeDetails['strSource'] != null || 
//           (recipeDetails['strTags'] != null && recipeDetails['strTags'].toString().isNotEmpty)) {
...
//                                           .toString()
//                                           .split(',')
//                                           .map<pw.Widget>((tag) => pw.Container(
...
//                         pw.SizedBox(height: 5),
//                         pw.Text(
//                           'Downloaded on ${DateTime.now().day}/${DateTime.now().month}/${DateTime.now().year}',
//                           style: pw.TextStyle(
//                             fontSize: 10,
//                             color: lightTextColor,
//                           ),
...
//   // Helper method for PDF metadata items
//   pw.Widget _buildPdfMetadataItem(String label, String value, PdfColor primaryColor, PdfColor bgColor) {
//     return pw.Container(
...
//      pw.Text(
//           value,
//           style: pw.TextStyle( // Removed the 'const' keyword here
//             fontSize: 12,
//             fontWeight: pw.FontWeight.bold,
//           ),
...
class _RecipeDetailScreenState extends State<RecipeDetailScreen>
  ...
  try {
    ...
    try {
      ...
      final imageFile = File('${tempDir.path}/recipe_image.jpg');
      ...
      if (response.statusCode == 200) {
        ...
        await Share.shareXFiles(
          ...
          text: recipeText.toString(),
          subject: 'Recipe: ${widget.mealName}',
        );
        return;
      }
    } catch (imageError) {
      debugPrint('Error sharing with image: $imageError');
      // Continue to text-only sharing if image sharing fails
    }
